...
class Network:
    def __init__(
        self, rpc: str, wallet: ChecksumAddress, min_pool_size_bnb: float, price_in_usd: bool, secrets
    ):
        self.wallet = wallet
    ...
    def get_token_price(self, token_address: ChecksumAddress) -> Tuple[Decimal, ChecksumAddress]:
        ...
        biggest_lp, lp_index,value_lp = self.find_biggest_lp(token, lps=supported_lps)
        ...
        # print('--------------------------------------------')
        return self.get_token_price_for_lp(token, base_token, ignore_poolsize=True), base_token_address,value_lp
    ...
    def get_token_price_for_lp(self, token: Contract, base_token: Contract, ignore_poolsize: bool = False) -> Decimal:
        ...
        return (Decimal(0) if value < 1e-30 else value,base_amount)  # artifact with small numbers
    ...
    def find_biggest_lp(
        ...
        # print((argmax,lp_balances))
        return lps[argmax], argmax,lp_balances[argmax]
